...
void tirarAlArco(jugador *j)
{
	...
	if(!*j->alguienEstaPateando)
	{
		*j->alguienEstaPateando = 1;
		printf("Soy el jugador %d y voy a disparar al arco %d.\n", j->id, MSG_JUG_EQUIPO_2);
		memset(mens, 0x00, sizeof(mens));
		sprintf(mens, "%d", j->tipo);
		enviarMensaje(idColaMensajes, MSG_JUG_EQUIPO_2, j->id, EVT_PATEA, mens);
...
pthread_mutex_lock(&mutex);				
		...
		else
		{
			...
			{
				printf("Esperando un tiro del equipo 2 \n");		
			}
		}
...
int main(int incant, char *szarg[])
{
	...
	memoria[0].equipo1Listo = 1;
	while(!memoria[0].equipo2Listo)
	{
		printf("Inicie al equipo 2 para poder comenzar \n");
		printf("Debido al retraso del equipo 2, el equipo 1 hara el primer saque.\n");
		memoria[0].primerEquipo = 1;
		puedeTirar = 1;	
		usleep(100);
	}
	...
	for(i = 0; i < 5; i++)
	{	
		jugadores[i].id = MSG_JUG_EQUIPO_1 + i;
